...
A interface usa cores diferentes para facilitar a apresentação:

```text
Amarelo     Título, bordas e eventos
Verde       Dados recebidos e estados ativos
Vermelho    Comandos enviados e erros
Magenta     Status e último valor
Ciano       Frames de controle recebidos
...
## Descobrir o nome da porta no Linux
...
Também é possível verificar antes e depois de conectar o ESP32:
...
Verifique se o cabo USB transmite dados. Alguns cabos servem apenas para alimentação.
...
Também pode ser necessário instalar ou verificar o driver do conversor USB serial usado pela placa.
...
A pasta `.venv` não deve ser enviada para o GitHub. Ela pode ser adicionada ao `.gitignore`:
